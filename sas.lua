...
--==================== FOV & WallCheck ====================
local function InFOV(worldPos, customFOV)
    local screenPos, onScreen = Camera:WorldToViewportPoint(worldPos)
    if not onScreen then return false end
    local mousePos = UIS:GetMouseLocation()
    local dist = (Vector2.new(screenPos.X, screenPos.Y) - mousePos).Magnitude
    return dist <= (customFOV or FOV_RADIUS)
end
...
local function WallCheck(targetHead, localChar, plr)
    ...
    if plr then
        ...
        if cached and (tick() - cached.time) < WALLCHECK_CACHE_TIME then
    ...
    if plr then
        wallCheckCache[plr] = {result = result, time = tick()}
    end
    ...
    Callback = function()
        ...
        connection = UIS.InputBegan:Connect(function(input, gameProcessed)
            if isRecordingKeybind and input.UserInputType == Enum.UserInputType.Keyboard then
                ...
                Rayfield:Notify({Title = "Keybind Set", Content = "Key: " .. aimlockKeyName, Duration = 2, Image = 4483362458})
            end
        end)
...
local ShowFOVToggle = RageTab:CreateToggle({
...
-- ColorPicker прикреплён к Toggle (правильный Rayfield API)
local FOVColorPicker = ShowFOVToggle:CreateColorPicker({
    Name = "FOV Color",
...
RageTab:CreateSection("Friends")

local FriendListLabel = RageTab:CreateLabel("Friend List: (empty)")
...
local ShowSilentFOVToggle = RageTab:CreateToggle({
...
-- ColorPicker прикреплён к Toggle
local SilentFOVColorPicker = ShowSilentFOVToggle:CreateColorPicker({
...
-- ESP Color прикреплён к HP Toggle
local ESPColorPicker = HPToggle:CreateColorPicker({
    Name = "ESP Color",
...
local NoFallToggle = MiscTab:CreateToggle({
    ...
    Callback = function(Value)
        if Value then
            task.spawn(startNoFall)
        else
            task.spawn(stopNoFall)
        end
    end,
})
...
--==================== INPUT HANDLERS ====================
-- ВАЖНО: это ПОСЛЕ создания AimlockKeybindLabel
...
local function createESPObjects(plr)
    cleanupPlayerESP(plr)
    local hpText = Drawing.new("Text"); hpText.Visible = false; hpText.Color = Settings.ESP_Color; hpText.Size = 14; hpText.Center = true; hpText.Outline = true
    ESP_HPText[plr] = hpText
    local nameText = Drawing.new("Text"); nameText.Visible = false; nameText.Color = Settings.ESP_Color; nameText.Size = 9; nameText.Center = true; nameText.Outline = true
    ESP_NameText[plr] = nameText
    local weaponText = Drawing.new("Text"); weaponText.Visible = false; weaponText.Color = Settings.ESP_Color; weaponText.Size = 12; weaponText.Center = true; weaponText.Outline = true
...
local function get3DBoxCorners(hrp)
    local cf = hrp.CFrame
    return {
        cf * Vector3.new(-2, 3, 0), cf * Vector3.new(2, 3, 0),
        cf * Vector3.new(-2, -3, 0), cf * Vector3.new(2, -3, 0),
    }
end
    ...
    if Box_ESP_Enabled and ESP_Boxes[plr] then
        ...
        if boxCorners and #boxCorners > 0 then
            ...
            if anyVisible then
                ...
                ESP_Boxes[plr].box.Position = Vector2.new(minX, minY)
                ESP_Boxes[plr].box.Size = Vector2.new(w, h)
                ESP_Boxes[plr].box.Color = color
                ESP_Boxes[plr].box.Visible = true
                ESP_Boxes[plr].boxoutline.Position = Vector2.new(minX - 1, minY - 1)
...
for _, plr in pairs(Players:GetPlayers()) do
    if plr ~= localPlayer then initPlayer(plr) end
end
...
local hookSuccess, hookError = pcall(function()
    ...
    if setreadonly then setreadonly(mt, false)
    elseif make_writeable then make_writeable(mt) end
...
if hookSuccess then
    Rayfield:Notify({Title = "Silent Aim", Content = "Hook installed", Duration = 3, Image = 4483362458})
else
    Rayfield:Notify({Title = "Silent Aim", Content = "Hook failed: " .. tostring(hookError), Duration = 5, Image = 4483362458})
end
