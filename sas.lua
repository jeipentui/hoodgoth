...
-- WallCheck для Silent Aim (с кастомным FOV)
local function WallCheckSilent(targetHead, localChar)
...
local function updateSilentAimTarget()
    ...
    for _, c in ipairs(candidates) do
        if WallCheckSilent(c.head, localChar) then
...
local DestroyUIButton = MiscTab:CreateButton({
    ...
    Callback = function()
        ...
        -- Восстанавливаем хук silent aim
        pcall(function()
