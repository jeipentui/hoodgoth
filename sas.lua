...
tabBorder=Color3.fromRGB(70,70,70),tabTriangle=Color3.fromRGB(85,85,85),
}
...
if c:IsA("Tool") then monitorTool(c) end
end)
char.ChildRemoved:Connect(function(c)
if c:IsA("Tool") then lastShotTime=0 end
end)
end
...
local no=r1.Position+(dir*0.5)
local rem=dist-(no-origin).Magnitude
if rem>0 then
local r2=workspace:Raycast(no,dir*rem,rp)
...
if t then return weaponBulletSpeeds[t.Name] or 1624 end
end
return 1624
end

local function isFriend(plr)
for _,n in pairs(FriendList) do
...
local cs=CharStats:FindFirstChild(tc.Name)
if not cs then return false end
local dv=cs:FindFirstChild("Downed")
if dv and dv:IsA("BoolValue") then return dv.Value end
return false
end
...
local mp=frameCache.mousePos
local pr=getFOVPixelRadius()
local cands={}
...
cands[#cands+1]={plr=plr,head=head,dist=d}
end
end
end
end
...
if WallCheck(c.head,lc,c.plr) then return c.head end
end
return nil
end
...
if isValidTarget(plr,currentTarget) and WallCheck(currentTarget,lp.Character,plr) then
...
local lc=lp.Character
if not lc then silentAimTarget=nil return end
local mp=frameCache.mousePos
local pr=getFOVPixelRadius()
local cands={}
for _,plr in pairs(Players:GetPlayers()) do
...
table.sort(cands,function(a,b) return a.dist<b.dist end)
for _,c in ipairs(cands) do
if WallCheck(c.head,lc,c.plr) then
...
elseif espVisualMode=="Off hotkey" then return not espVisualKeyHeld
end
return false
end
...
ESP_HPText[plr]=Drawing.new("Text")
ESP_HPText[plr].Visible=false
ESP_HPText[plr].Color=Settings.HP_Color
ESP_HPText[plr].Size=14
ESP_HPText[plr].Center=true
ESP_HPText[plr].Outline=true

ESP_NameText[plr]=Drawing.new("Text")
ESP_NameText[plr].Visible=false
ESP_NameText[plr].Color=Settings.Name_Color
ESP_NameText[plr].Size=9
ESP_NameText[plr].Center=true
ESP_NameText[plr].Outline=true

ESP_WeaponText[plr]=Drawing.new("Text")
ESP_WeaponText[plr].Visible=false
ESP_WeaponText[plr].Color=Settings.Weapon_Color
ESP_WeaponText[plr].Size=12
ESP_WeaponText[plr].Center=true
ESP_WeaponText[plr].Outline=true
...
local bxo=Drawing.new("Square")
bxo.Visible=false
bxo.Thickness=1
bxo.Color=Color3.new(0,0,0)
...
lastDistanceCheck=frameCache.time
local cp=frameCache.cameraPos
for _,plr in pairs(Players:GetPlayers()) do
if plr~=lp and plr.Character then
...
local h=char:FindFirstChild("Humanoid")
if not h or h.Health<=0 then cleanupPlayerESP(plr) return end
...
ESP_HPText[plr].Color=ESP_HPDynamicEnabled and Color3.fromHSV((hp/h.MaxHealth)/3,1,1) or hpCol
...
ESP_NameText[plr].Color=nameCol
...
ESP_WeaponText[plr].Color=weapCol
...
if bc and #bc>0 then
local mnX,mnY,mxX,mxY=9e9,9e9,-9e9,-9e9
local av=false
for _,c in ipairs(bc) do
if c.visible and c.z>0 then
av=true
if c.pos.X<mnX then mnX=c.pos.X end
if c.pos.Y<mnY then mnY=c.pos.Y end
if c.pos.X>mxX then mxX=c.pos.X end
if c.pos.Y>mxY then mxY=c.pos.Y end
end
end
if av then
local w,hh=mxX-mnX,mxY-mnY
ESP_Boxes[plr].box.Position=Vector2.new(mnX,mnY)
ESP_Boxes[plr].box.Size=Vector2.new(w,hh)
ESP_Boxes[plr].box.Color=boxCol
ESP_Boxes[plr].box.Visible=true
ESP_Boxes[plr].boxoutline.Position=Vector2.new(mnX-1,mnY-1)
ESP_Boxes[plr].boxoutline.Size=Vector2.new(w+2,hh+2)
ESP_Boxes[plr].boxoutline.Color=Color3.new(0,0,0)
...
TileSize=UDim2.fromOffset(tile or 6,tile or 6),
ImageColor3=colorB or Color3.fromRGB(20,20,20)
},parent)
mk("Frame",{
Name="DotBase",
...
local function createTabMarker(target)
local marker=mk("Frame",{
Size=UDim2.fromOffset(7,7),
Position=UDim2.new(0,-4,1,-3),
BackgroundColor3=C.tabTriangle,
BorderSizePixel=0,
ZIndex=(target.ZIndex or 1)+2
},target)
marker.Rotation=45
return marker
end
...
local needMarker=(index==2 or index==3 or index==5 or index==7 or index==8)
if needMarker then
createTabMarker(pageBorder)
end
...
createGroup(parent, 6 + 250 + 20, 20, 250, 250, "Fake lag")
createGroup(parent, 6 + 250 + 20, 20 + 250 + 20, 250, 250, "Other")
end
...
function(mode) espVisualMode=mode end,
...
createGroup(parent, 6, 5, 250, 510, "Miscellaneous")
createGroup(parent, 281, 5, 250, 245, "Movement")
createGroup(parent, 281, 270, 250, 245, "Settings")
end

local function buildSkinsPage(parent)
createGroup(parent, 6, 20, 250, 500, "Model options")
createGroup(parent, 276, 20, 250, 500, "Weapon skin")
end
...
local adjustY=24
local whitelistCB=createCheckbox(rightContent,adjustY,"Add to whitelist",false,function(v)
if playerPageData.selectedPlayer then
local ps=getPlayerSettings(playerPageData.selectedPlayer)
ps.whitelisted=v
if v then
local found=false
for _,n in pairs(FriendList) do if n==playerPageData.selectedPlayer then found=true break end end
if not found then table.insert(FriendList,playerPageData.selectedPlayer) end
else
for i,n in pairs(FriendList) do if n==playerPageData.selectedPlayer then table.remove(FriendList,i) break end end
end
end
end)
adjustY=adjustY+22
local teamColorCB=createCheckbox(rightContent,adjustY,"Team color",false,function(v)
if playerPageData.selectedPlayer then
local ps=getPlayerSettings(playerPageData.selectedPlayer)
ps.teamColor=v
end
...
adjustY=adjustY+22
local disableVisCB=createCheckbox(rightContent,adjustY,"Disable visuals",false,function(v)
if playerPageData.selectedPlayer then
local ps=getPlayerSettings(playerPageData.selectedPlayer)
ps.disableVisuals=v
if v then
local plr=Players:FindFirstChild(playerPageData.selectedPlayer)
if plr then hidePlayerESP(plr) end
end
end
end)
adjustY=adjustY+28
...
if plr~=lp then
if search=="" or plr.Name:lower():find(search,1,true) then
...
local ps=getPlayerSettings(plr.Name)
local nameCol=ps.whitelisted and Settings.Friend_Color or C.text
...
if playerPageData.selectedPlayer~=plr.Name then btn.BackgroundColor3=Color3.fromRGB(28,28,28) end
end)
btn.MouseLeave:Connect(function()
if playerPageData.selectedPlayer~=plr.Name then btn.BackgroundColor3=Color3.fromRGB(18,18,18) end
end)
btn.MouseButton1Click:Connect(function()
playerPageData.selectedPlayer=plr.Name
selectedLabel.Text=plr.Name
selectedLabel.TextColor3=C.text
local s=getPlayerSettings(plr.Name)
whitelistCB.setEnabled(s.whitelisted)
teamColorCB.setEnabled(s.teamColor or false)
disableVisCB.setEnabled(s.disableVisuals)
for _,b in pairs(playerPageData.buttons) do
b.BackgroundColor3=(b.Name==plr.Name) and Color3.fromRGB(35,35,35) or Color3.fromRGB(18,18,18)
end
end)
...
Players.PlayerRemoving:Connect(function(plr)
if playerPageData.selectedPlayer==plr.Name then
playerPageData.selectedPlayer=nil
selectedLabel.Text="No player selected"
selectedLabel.TextColor3=C.textDim
end
task.wait(0.1)
refreshPlayerList()
end)
...
createGroup(parent, 276, 20, 250, 500, "Lua")
end
...
ColorSequenceKeypoint.new(0,Color3.fromRGB(55,170,255)),
...
elseif espVisualMode=="On hotkey" then
espVisualKeyHeld=true
elseif espVisualMode=="Off hotkey" then
espVisualKeyHeld=true
if not isESPActive() then
for plr,_ in pairs(ESP_HPText) do hidePlayerESP(plr) end
end
...
if espVisualKey and input.KeyCode==espVisualKey then
if espVisualMode=="On hotkey" then
espVisualKeyHeld=false
if not isESPActive() then
for plr,_ in pairs(ESP_HPText) do hidePlayerESP(plr) end
end
elseif espVisualMode=="Off hotkey" then
...
fovCircle.Position=frameCache.mousePos
...
cacheViewportPoints()
for plr,inR in pairs(playersInRange) do
if inR then updatePlayerESP(plr) else hidePlayerESP(plr) end
end
end)
