...
local function openContextMenu(anchorFrame,modes,getCurrentMode,onSelect)
	...
	local anchorAbs=anchorFrame.AbsolutePosition
	local mainAbs=main.AbsolutePosition
	local relX=anchorAbs.X-mainAbs.X-ctxW-5
	local relY=anchorAbs.Y-mainAbs.Y

	local ctx=mk("Frame",{Size=UDim2.fromOffset(ctxW,ctxH),Position=UDim2.fromOffset(relX,relY),BackgroundColor3=C.contextBg,BorderSizePixel=0,ZIndex=500},main)
	...
	for i,mode in ipairs(modes) do
		...
		itemBtn.MouseButton1Down:Connect(function()
			...
			task.defer(function() closeActiveContext() end)
		end)
	end
...
local function createCheckboxWithColor(parent,yPos,labelText,defaultValue,defaultColor,onToggle,onColorChanged)
	...
	previewBtn.MouseButton1Click:Connect(function()
		...
		local relX=anchorAbs.X-mainAbs.X-PS-HW-16;local relY=anchorAbs.Y-mainAbs.Y-4

		pickerFrame=mk("Frame",{Size=UDim2.fromOffset(PS+HW+12,PS+8),Position=UDim2.fromOffset(relX,relY),BackgroundColor3=C.contextBg,BorderSizePixel=0,ZIndex=600},main)
